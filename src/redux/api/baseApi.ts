...
const baseQuery = fetchBaseQuery({
    baseUrl: 'https://stationary-shop-server-6kg4nopj3-moniruzzaman2525s-projects.vercel.app/api',
    credentials: 'include',
...
const baseQueryRefreshToken: BaseQueryFn<FetchArgs, BaseQueryApi, DefinitionType> = async (args, api, extraOption): Promise<any> => {
    ...
    if (result?.error?.status === 401) {
        const res = await fetch('https://stationary-shop-server-6kg4nopj3-moniruzzaman2525s-projects.vercel.app/api/auth/refresh-token', {
            method: 'POST',
            credentials: 'include'
        })
